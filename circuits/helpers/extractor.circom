...
template TimestampExtractor(maxDataLength) {
    ...
    component dateToUnixTime = DigitBytesToTimestamp(2050);
...
template AgeExtractor(maxDataLength) {
    ...
    // DOB always appears near the start of the payload, so a 256-byte window is sufficient.
    component shifter = VarShiftLeft(maxDataLength, 256);
    shifter.in <== nDelimitedData;
    shifter.shift <== startDelimiterIndex; // We want delimiter to be the first byte

    signal shiftedBytes[256] <== shifter.out;
...
/// @title AgeOnlyExtractor
/// @notice Extracts age-only claim signals from Aadhaar QR data for fast proving
...
/// @output ageYears - Computed age in full years
/// @output ageYears - Computed age in full years
template AgeOnlyExtractor(maxDataLength) {
    signal input data[maxDataLength];
    signal input delimiterIndices[18];

    signal output timestamp;
    signal output ageYears;
    ...
    // Extract age in years. Threshold comparison is done in main circuit via minAge input.
    ...
    ageYears <== ageExtractor.age;

    // Photo/nullifier extraction is intentionally skipped in age-only mode.
}

/// @title IdentityAgeExtractor
/// @notice Extracts age/timestamp plus bounded name/photo slices for identity binding.
template IdentityAgeExtractor(maxDataLength) {
    signal input data[maxDataLength];
    ...
    signal output timestamp;
    signal output ageYears;
    signal output nameBytes[64];

    // Build nDelimitedData so each delimiter is replaced by (delimiterCount * 255).
    component is255[maxDataLength];
    component indexBeforePhoto[maxDataLength];
    signal is255AndIndexBeforePhoto[maxDataLength];
    signal nDelimitedData[maxDataLength];
    ...
    // Enforce that delimiterIndices[2] and [3] actually surround the name field.
    component nameStartDelimiter = ItemAtIndex(maxDataLength);
    nameStartDelimiter.in <== nDelimitedData;
    nameStartDelimiter.index <== delimiterIndices[namePosition() - 1];
    nameStartDelimiter.out === namePosition() * 255;

    component nameEndDelimiter = ItemAtIndex(maxDataLength);
    nameEndDelimiter.in <== nDelimitedData;
    nameEndDelimiter.index <== delimiterIndices[namePosition()];
    nameEndDelimiter.out === (namePosition() + 1) * 255;

    component nameExtractor = SelectSubArray(maxDataLength, 64);
    nameExtractor.in <== data;
    nameExtractor.startIndex <== delimiterIndices[namePosition() - 1] + 1;
    nameExtractor.length <== delimiterIndices[namePosition()] - delimiterIndices[namePosition() - 1] - 1;
    nameBytes <== nameExtractor.out;

}

/// @title QRDataExtractor
/// @notice Backward-compatible alias to the age-only extractor.
template QRDataExtractor(maxDataLength) {
    ...
    component ageOnlyExtractor = AgeOnlyExtractor(maxDataLength);
    ageOnlyExtractor.data <== data;
    ageOnlyExtractor.delimiterIndices <== delimiterIndices;

    timestamp <== ageOnlyExtractor.timestamp;
    ageYears <== ageOnlyExtractor.ageYears;
}
